...
SAMPLES="Montipora_selected_Runs.samples.txt"
REF="../00_databases/Montipora_capitata_KBHIv3.assembly.fasta"
...
RNAseq_samples_readmapping() {
	...
	## 02 - FastqToSam + addReadGroups + MergeBamAlignment
	...
	## Add read group info to unaligned bam file.
	log "Step 2.2 - addReadGroups"
	if [ ! -f "${OUT}.addReadGroups.done" ];
	then
		run_cmd "gatk AddOrReplaceReadGroups \
				--INPUT ${OUT}.FastqToSam.unmapped.bam \
				--OUTPUT ${OUT}.FastqToSam.unmapped.rg.bam \
				--RGPL illumina \
				--RGLB ${OUT} \
				--RGPU ${OUT} \
				--RGSM ${OUT} \
				--RGID ${OUT}" && \
		run_cmd "touch ${OUT}.addReadGroups.done"
	else
		log "         - Already Fishied!"
	fi
